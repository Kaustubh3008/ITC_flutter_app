...
class _ServerUploadScreenState extends State<ServerUploadScreen> {
  ...
  // --- 1. SMART SCAN (FIXED FOR YOUR PATH) ---
  Future<void> _loadSmartList() async {
    setState(() => _isLoading = true);
    // Request permissions (Android only)
    await [Permission.storage, Permission.manageExternalStorage].request();
    ...
    // Correct Path based on your request
    String androidPath = '/storage/emulated/0/Download/AgriSync_Files';
    String laptopPath = r'C:\Users\kaust\OneDrive\Documents\xyzitc';

    List<String> pathsToCheck = [laptopPath, androidPath];
      ...
      if (await rootDir.exists()) {
        try {
          ...
          for (var entity in entities) {
            // Case A: It's the Zip file directly (Flat structure)
            if (entity is File && entity.path.toLowerCase().endsWith('.zip')) {
              foundItems.add(entity);
            }
            // Case B: It's a Batch Folder (Nested structure: BatchFolder > Zip)
            else if (entity is Directory) {
              try {
                // Look INSIDE the batch folder for the zip file
                List<FileSystemEntity> subContents = entity.listSync();
                var zips = subContents.where(
                  (e) => e is File && e.path.toLowerCase().endsWith('.zip'),
                );

                // Add the ZIP file to our list, not the folder
                foundItems.addAll(zips);
              } catch (e) {
                print("Error scanning subdirectory: $e");
              }
            }
          }
        } catch (e) {
  ...
  // --- 2. LOGIC: HANDLING THE ZIP FILE ---
  Future<void> _processAndUpload() async {
    if (_selectedItem == null) return;

    // Check if selected item is actually a file
    if (_selectedItem is! File) {
      _showErrorDialog("Selected item is not a file.");
      return;
    }

    File zipFile = _selectedItem as File;

    setState(() {
      _isUploading = true;
      _uploadStatus = "Uploading Zip File...";
    });

    try {
      print("--- [DEBUG] Uploading File: ${zipFile.path} ---");

      // DIRECT UPLOAD - No unzipping, no re-zipping
      final result = await ApiService.uploadBatch(zipFile);

      if (result['status'] == 'success') {
        _showSuccessDialog("Upload Success", "Job ID: ${result['job_id']}");
      } else {
        _showErrorDialog("Upload Failed: ${result['message']}");
      }
    } catch (e) {
      _showErrorDialog("Process Stopped: $e");
    } finally {
        ...
        children: [
          Expanded(
            child: _isLoading
                ...
                : _allFiles.isEmpty
                ? const Center(child: Text("No Zip Files Found"))
                : ListView.builder(
                    padding: const EdgeInsets.all(16),
                    itemCount: _allFiles.length,
                    itemBuilder: (context, index) {
                      ...
                      return GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(sizeStr), // Show file size
                            trailing: isSelected
                                ? const Icon(
                                    Icons.check_circle,
                                    color: Color(0xFF2E7D32),
